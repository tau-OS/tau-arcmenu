...
var RecentFilesManager = class ArcMenu_RecentFilesManager {
    ...
    filterRecentFiles(callback){
        let queryPromises = this.createQueryPromises();
    ...
    createQueryPromises(){
        ...
        recentManagerItems.forEach(item => {
            promises.push(this._createQueryPromise(item));
        });

        return promises;
    }

    _createQueryPromise(item) {
            ...
            file.query_info_async('standard::type,standard::is-hidden', 0, 0, cancellable, (source, res) => {
                try {
                    ...
                    if (fileInfo) {
                        ...
                        reject('No File Info Found. Rejected')
                }
                catch (err) {
